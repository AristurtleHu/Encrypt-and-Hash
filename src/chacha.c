...
#pragma omp parallel for
  for (size_t block_idx = 0; block_idx < num_blocks; block_idx += 2) {
    ...
    // XOR for the first block in the pair (block_idx)
    size_t offset = block_idx * 64;
    if (offset < length) {
      size_t remaining = length - offset;
      size_t bytes_in_block0 = (remaining < 64) ? remaining : 64;
      xor_buffer_simd(buffer + offset, key_stream_pair, bytes_in_block0);
    }

    // XOR for the second block in the pair (block_idx + 1)
    if (block_idx + 1 < num_blocks) {
      offset = (block_idx + 1) * 64;
      if (offset < length) {
        size_t remaining = length - offset;
        size_t bytes_in_block1 = (remaining < 64) ? remaining : 64;
        xor_buffer_simd(buffer + offset, key_stream_pair + 64, bytes_in_block1);
      }
    }
  }
}
